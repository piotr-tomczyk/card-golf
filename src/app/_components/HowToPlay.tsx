...
export function HowToPlay() {
  ...
  return (
    <div className={`${playfair.variable} w-full max-w-5xl`}>
      {/* Toggle button */}
      <button
        onClick={() => setOpen((v) => !v)}
      ...
      >
        ...
        {open ? t("close") : t("toggle")}
        <span
          className={`ml-1 text-xs transition-transform duration-300 ${open ? "rotate-180" : ""}`}
        >
          ▾
        </span>
      </button>
      ...
      <div
        className={`overflow-hidden transition-all duration-500 ease-in-out ${
          open ? "max-h-[9999px] opacity-100 mt-4" : "max-h-0 opacity-0"
        }`}
      >
        <div
          className="rounded-2xl border border-amber-900/30 bg-green-950 shadow-2xl"
          style={{
            backgroundImage:
        ...
        >
          {/* Header */}
          <div className="border-b border-amber-800/25 px-6 py-5 text-center">
