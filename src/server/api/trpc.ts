...
/**
 * Player procedure - accepts either authenticated users or guests.
 * Guarantees ctx.playerId and ctx.playerName are available.
 */
export const playerProcedure = t.procedure
  ...
  .use(({ ctx, next }) => {
    ...
    if (authUser) {
      return next({
        ctx: {
          ...
          playerName: authUser.name ?? "Player",
    ...
    if (guestId) {
      return next({
        ctx: {
          playerId: `guest_${guestId}`,
          playerName: ctx.guestName ?? "Guest",
    ...
    throw new TRPCError({
      code: "UNAUTHORIZED",
      message: "Must be logged in or playing as guest",
    });
  });
